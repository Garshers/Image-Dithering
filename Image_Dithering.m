...
function dithered_image = floyd_steinberg_dithering(gray_image, threshold_level)
    gray_image = double(gray_image) / 255; % Normalize to [0, 1]
    [height, width] = size(gray_image);
    dithered_image = gray_image;
    for y = 1:height
        for x = 1:width
            old_pixel = dithered_image(y, x);
            new_pixel = round(old_pixel); % Binarization
            dithered_image(y, x) = new_pixel;
            error = old_pixel - new_pixel;
            if x < width
                dithered_image(y, x+1) = dithered_image(y, x+1) + error * 7/16;
            end
            if x > 1 && y < height
                dithered_image(y+1, x-1) = dithered_image(y+1, x-1) + error * 3/16;
            end
            if y < height
                dithered_image(y+1, x) = dithered_image(y+1, x) + error * 5/16;
            end
            if x < width && y < height
                dithered_image(y+1, x+1) = dithered_image(y+1, x+1) + error * 1/16;
            end
        end
    end
    dithered_image = dithered_image > threshold_level; % Convert to binary image
end
